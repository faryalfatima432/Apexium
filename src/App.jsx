...
export default function App() {
  ...
  const buyNow = (product, quantity = 1) => {
    setBuyNowItem({ ...product, quantity });
    window.location.href = isLoggedIn ? '/checkout' : '/my-account?buyNow=true';
  };
  ...
  const userLogin = () => setIsLoggedIn(true);
  const userLogout = () => {
    setIsLoggedIn(false);
    setBuyNowItem(null);
    localStorage.removeItem('userLoginData');
  ...
  return (
    <CartContext.Provider value={{
      cartItems, addToCart, buyNow, updateQuantity,
      removeFromCart, clearCart, toggleCart, closeCart,
      isLoggedIn, userLogin, userLogout, buyNowItem
    }}>
      <div className="App">
        {!isAdminRoute && (
          <Header
